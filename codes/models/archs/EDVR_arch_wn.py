...
from  models.archs import arch_util_wn
from models.archs.arch_util_wn import wn
...
class Predeblur_ResNet_Pyramid(nn.Module):
    def __init__(self, nf=128, HR_in=False):
        ...
        basic_block = functools.partial(arch_util_wn.ResidualBlock_noBN, nf=nf)
...
class TSA_Fusion(nn.Module):
    ...
    def __init__(self, nf=64, nframes=5, center=2):
        ...
        self.tAtt_1 = wn(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
        self.tAtt_2 = wn(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))

        # fusion conv: using 1x1 to save parameters and computation
        self.fea_fusion = wn(nn.Conv2d(nframes * nf, nf, 1, 1, bias=True))

        # spatial attention (after fusion conv)
        self.sAtt_1 = wn(nn.Conv2d(nframes * nf, nf, 1, 1, bias=True))
        self.maxpool = nn.MaxPool2d(3, stride=2, padding=1)
        self.avgpool = nn.AvgPool2d(3, stride=2, padding=1)
        self.sAtt_2 = wn(nn.Conv2d(nf * 2, nf, 1, 1, bias=True))
        self.sAtt_3 = wn(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
        self.sAtt_4 = wn(nn.Conv2d(nf, nf, 1, 1, bias=True))
        self.sAtt_5 = wn(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
        self.sAtt_L1 = wn(nn.Conv2d(nf, nf, 1, 1, bias=True))
        self.sAtt_L2 = wn(nn.Conv2d(nf * 2, nf, 3, 1, 1, bias=True))
        self.sAtt_L3 = wn(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
        self.sAtt_add_1 = wn(nn.Conv2d(nf, nf, 1, 1, bias=True))
        self.sAtt_add_2 = wn(nn.Conv2d(nf, nf, 1, 1, bias=True))
...
class EDVR(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        self.HR_in = True if HR_in else False
        self.w_TSA = w_TSA
        ResidualBlock_noBN_f = functools.partial(arch_util_wn.ResidualBlock_noBN, nf=nf)
        ...
        self.feature_extraction = arch_util_wn.make_layer(ResidualBlock_noBN_f, front_RBs)
        self.fea_L2_conv1 = wn(nn.Conv2d(nf, nf, 3, 2, 1, bias=True))
        self.fea_L2_conv2 = wn(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
        self.fea_L3_conv1 = wn(nn.Conv2d(nf, nf, 3, 2, 1, bias=True))
        self.fea_L3_conv2 = wn(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
        ...
        #### reconstruction
        self.recon_trunk = arch_util_wn.make_layer(ResidualBlock_noBN_f, back_RBs)
        #### upsampling
