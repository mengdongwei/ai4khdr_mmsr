...
def main():
    dataset = 'hdr'  # vimeo90K | REDS | general (e.g., DIV2K, 291) | DIV2K_demo |test
...
def general_image_folder(opt):
    ...
    if isinstance(img_folder, list):
        all_img_list = []
        for  xx in img_folder:
            all_img_list = all_img_list + glob.glob(osp.join(xx, '*'))
        all_img_list.sort()
    else:
...
def hdr_train(mode):
    ...
    BATCH = 5000  # After BATCH images, lmdb commits, if read_all_imgs = False
    if mode == 'GT':
        img_folder = '/workspace/datasets/AI_4K_HDR/train/4k_frames'
        lmdb_save_path = '/workspace/datasets/AI_4K_HDR/train_4k.lmdb'
        txt_file = '/workspace/datasets/AI_4K_HDR/train/4k_sep_trainlist.txt'
        H_dst, W_dst = 256, 448
    elif mode == 'LR':
        img_folder = '/workspace/datasets/AI_4K_HDR/train/540p_frame'
        lmdb_save_path = '/workspace/datasets/AI_4K_HDR/train_540p.lmdb'
        txt_file = '/workspace/datasets/AI_4K_HDR/train/540p_sep_trainlist.txt'
        H_dst, W_dst = 64, 112
    elif mode == 'flow':
